...
import utils
from model import TITFormer, Enhancer
...
def train():
    args = OmegaConf.load("option/trainppr.yaml")
    trainOpt = args.train
        ...
        for i, batch in enumerate(loop):
            ...
            if i % trainOpt.print_each_step == 0:
                loop.set_description(
                    "t| E:{}/{}loss:{:.4f},psnr:{:.4f}"
                        .format(epoch, trainOpt.n_epochs, training_meter['loss'].avg, training_meter['psnr'].avg, ))
        ...
        for i, batch in enumerate(loopVal):
            ...
            if i % trainOpt.print_each_step == 0:
                loopVal.set_description(
                    "v|E: {}/{} loss:{:.4f},psnr:{:.4f}"
                        .format(epoch, trainOpt.n_epochs, val_average['loss'].avg, val_average['psnr'].avg))
